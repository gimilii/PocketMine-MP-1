...
// 							$array = &$this->zombie;
							$array = &$this->mob;   // DAF Should be able to remove all these ifs and just check instanceOf Monster
...
// 			}
			// DAF Should be able to remove all these ifs and just check instanceOf Monster
			if(isset($this->mob[$eid])){
				unset($this->mob[$eid]);
			}
		}
...
// 				$array = &$this->zombie;
	...
	public function knockBackover(Entity $entity, Vector3 $v3){
		if($entity instanceof Entity){
			// DAF Should be able to remove all these ifs and just check instanceOf Monster
			if(isset($this->mob[$entity->getId()])){
				$entity->setPosition($v3);
				$this->mob[$entity->getId()]['knockBack'] = false;
			}
	...
	public static function entityMotion(Creature $creature, $x, $y, $z) {
		$p = new SetEntityMotionPacket;
		$p->entityRuntimeId = $creature->getId();
		$p->motionX = $x;
		$p->motionY = $y;
		$p->motionZ = $z;
		foreach ($creature->getViewers() as $pl) {
			$pl->dataPacket($p);
		}
	}
}
